...
- `git` and `openssh-client` are installed for users who point `BEANCOUNT_INPUT_FILE` at a ledger kept in a repository
  (e.g. cloned by an init container), over `https://` or `ssh://`.
